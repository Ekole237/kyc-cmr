...
function ReviewCard({ item }: { item: KycCase }) {
  ...
  return <Pressable onPress={() => router.push({ pathname: "/cases/[id]", params: { id: item.id } })} style={({ pressed }) => [styles.card, pressed && styles.pressed]}>
    <View style={styles.cardHeader}><View style={styles.cardIdentity}><Text numberOfLines={1} style={styles.name}>{item.fullName}</Text><Text numberOfLines={1} style={styles.meta}>{item.id} · {item.city}</Text></View><RiskPill risk={item.riskLevel} /></View>
